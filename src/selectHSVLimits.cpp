...
public:
	...
	//TODO check if these values are initialized or not
    	float *minH, *maxH, *minS,  *maxS;
	...
	void extractHsvBasedROI(float hsv[][3], int cloudSize, pcl::PointIndices::Ptr inliers) {
		...
		for (int i = 0; i < cloudSize; i++){
			...
			if(	hsv[i][0] >=*minH && hsv[i][0] <=*maxH &&
					hsv[i][1] >=*minS && hsv[i][1] <=*maxS )
	...
	void kinectCloudRawCallback(const sensor_msgs::PointCloud2 &cloud){
		...
		for(int i=0; i<cloudSize; i++){//i<10; i++){//
			...
			if(rgb[i][0] < 235) rgb[i][0] += 20;
			if(rgb[i][1] < 235) rgb[i][1] += 20;
			if(rgb[i][2] < 235) rgb[i][2] += 20;
...
int main(int argc, char** argv){
	...
	string userInput;
	cout << "Do you want to save the configuration ??  "<< endl;
	cin  >> userInput;
